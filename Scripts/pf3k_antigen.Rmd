...
```{r data}
pf3kv5 <- seqOpen("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/pf3kv5_with_PNG.pass.snps.snpeff_ann.core.gds", readonly = TRUE)

# geographic metadata
metadata <- read.delim("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/metadata/pf3k_release_5_png_metadata_collated.txt",
                       header=TRUE, stringsAsFactors=FALSE, na.strings=c(""))
rownames(metadata) <- metadata$sample

# target antigens
antigens <- read.delim("/wehisan/general/academic/lab_barry/Myo/Bourama_Genes/meta_data/target_gene.txt", header=TRUE, stringsAsFactors=FALSE)
...
func_annotations <- readr::read_rds("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/report/pf3kv5_functional_annotations_filtered.rds")
func_annotations <- func_annotations[!grepl("synonymous_variant",func_annotations$annotation),]
gatk_annotations <- readr::read_rds("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/report/pf3kv5_gatk_metrics.rds")
...
moi <- readr::read_rds("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/report/pf3kv5_fws.rds")
...
for (gene in antigens$Gene.ID) {
  ...
  seqGDS2VCF(pf3kv5, paste0("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/vcf/", gene, "_coding.vcf"),
  ...
  cds_seq <- read.dna(paste0("/wehisan/general/academic/lab_barry/Myo/Zalak_Analysis/cds_fasta", gene, ".1.fasta"), format="fasta") %>% as.character %>% as.vector %>% toupper
  # generate a fasta file containing coding sequences for each clone 
  generate_fasta(cds_seq, gene_var$pos_CDS, moi1_haps, moi2_seqs, paste0("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/fasta/", gene, "_CDS.fasta"), antigens[gene, ]$dir)
  ...
  if (nrow(gene_var)>10) {
      generate_pedmap(gene_var, moi1_haps, moi2_haps, metadata, paste0("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/pedmap/", gene, "_coding.ped"), paste0("/wehisan/general/academic/lab_barry/Somya/Pf3k_PNG/pedmap/", gene, "_coding.map"))
  }
